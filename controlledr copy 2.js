...
function updateMarkerAlongRoute(coordinates) {
  ...
  // Add the button for navigation
  const navigateButton = document.createElement('button');
  navigateButton.textContent = 'Navigate';
  navigateButton.addEventListener('click', () => {
    // Calculate and display the route from userLocation to point.coordinates
    setRouteFromStartToEnd(userLocation, point.coordinates);
  });
...
var followUser = true; // Flag to indicate whether the map should follow the user's location
...
if (navigator.geolocation) {
  ...
  var watchId = navigator.geolocation.watchPosition(
    (position) => {
      ...
      // Center the map on the user's location if followUser is true
      if (followUser) {
...
// Event listener for recenter button
document.getElementById("recenterBtn").addEventListener("click", () => {
  // Center the map on the user's location if followUser is true
  if (followUser && userLocation) {
    ...
    console.log("User location is not available or followUser is false.");
  }
});

// Event listener for toggle followUser button
