...
// Function to continuously recenter the map based on user location when a route is chosen
function recenterMapOnUserLocation() {
  // Check if there's a route on the map
  if (!map.getSource('route')) {
    console.log('No route found on the map.');
    return;
  }

  // Function to update map center
  function updateMapCenter() {
    // Check if the route is still on the map
    if (!map.getSource('route')) {
  ...
  // Update the map center initially
  updateMapCenter();

  // Set an interval to update the map center every 10 seconds (or any interval you prefer)
  const updateInterval = setInterval(updateMapCenter, 1000);

  // Optionally, you can store the interval ID to clear it later when the route is removed
...
// Call this function when the route is chosen
