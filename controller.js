...
// Function to continuously recenter the map when a route is chosen
function continuouslyRecenterMap() {
  if (routeAdded) {
    // Watch the user's position and update it continuously
    ...
    console.log("No route chosen."); // Log that no route is chosen
  }
}

let routeAdded = false;

// Call the function to continuously recenter the map
continuouslyRecenterMap();

// Function to toggle continuous recentering of the map
let recenterEnabled = false; // Initialize as disabled by default

function toggleRecenterMap() {
  if (recenterEnabled) {
    // Disable continuous recentering
    navigator.geolocation.clearWatch(watchId);
    recenterEnabled = false;
  } else {
    // Enable continuous recentering only if a route is added
    if (routeAdded) {
      continuouslyRecenterMap();
...
// Add event listener to the button to toggle continuous recentering
document.getElementById('toggleRecenterBtn').addEventListener('click', toggleRecenterMap);
