...
if (navigator.geolocation) {
  var watchId = navigator.geolocation.watchPosition(
    (position) => {
      ...
      updateDistances(userLocation);
      continuouslyRecenterMap(userLocation, !chosenDestination); // Recenter map only if a destination is not chosen
...
function continuouslyRecenterMap(userLocation, recenter) {
  if (recenter) {
    map.flyTo({
      center: userLocation,
      essential: true
  ...
  continuouslyRecenterMap(userLocation, true);
} else {
  continuouslyRecenterMap(userLocation, false);
}
